...
const PropertyTable: React.FC<IPropertyTable> = (props) => {
  ...
  const DisplayDataAll = () => {
    ...
    let columns: any[] = [
      {
        ...
        render: (value, row, index) => (
          <DragHandle value={value} row={row} index={index} />
        ),
      },
      {
        title: "Curve",
    ...
    const columnColor = paramVec.map((r) => {
      const monotonus_inc = r.value.every((e, i, a) => {
        if (i) return e >= a[i - 1];
        else return true;
      });
      const monotonus_dec = r.value.every((e, i, a) => {
        if (i) return e <= a[i - 1];
        else return true;
      });
      if (monotonus_inc || monotonus_dec)
        return { name: r.name, color: "green" };
      else return { name: r.name, color: "red" };
    });
    const columnStyle = (paramName: string) => {
      const e = columnColor.find((r) => r.name === paramName);
      if (e) return { color: e.color, textAlign: "center" };
      else return {};
    };
    ...
    const SortableContainer2 = SortableContainer((props) => (
      <tbody {...props} />
    ));
